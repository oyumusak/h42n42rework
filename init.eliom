...
let _moveMonster monster data =
	...
	if monster.yPos +. monster.size < float_of_int (areaHeight / 5) && monster.typee = Healthy then make_sick monster;
...
let rec gameLoop data =
	...
	if List.exists (fun x -> x.typee = Healthy) data.monsters = false then (
		let _ = Js.Unsafe.fun_call (Js.Unsafe.js_expr "alert") [|Js.Unsafe.inject (Js.string "Game Over!")|] in
		Lwt.return ()
	)
	else (
...
let initalizer () =
	...
	let deneme = ~%gameArea in
	Html.Manip.appendChild deneme river;
	Html.Manip.appendChild deneme hospital;

	let counterDiv = div ~a:[a_class ["creetcounter"]] [txt "creet count = 0"] in
	Html.Manip.appendChild deneme counterDiv;
	let data : gameData = {loopCount = 0; speedIter = 0.; monsters = []; lastId = 1; counterelt = counterDiv;} in

	let monster = createMonster data in
	Html.Manip.appendChild deneme monster.elt;
	data.monsters <- monster :: data.monsters;
